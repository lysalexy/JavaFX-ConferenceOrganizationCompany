...
public class AddBuffetController {
    @FXML
    private TextField amount;
    @FXML
    private ComboBox dishName;
    @FXML
    ...
    public void setInfo() {
        ...
        ObservableList<String> dishesNames = FXCollections.observableArrayList();
        for (Dish dish : dishes) {
            dishesNames.add(dish.getDishName());
        }

        dishName.setItems(dishesNames);
        ////задаем возмможные блюда в комбобокс
    }
    ...
    public void add() {
        try {
            ...
            String name = dishName.getValue().toString();
                DishRepository dishRepository = new DishRepository(connection);
                int dishId = dishRepository.getDishByDishName(dishName.getValue().toString());
                BuffetPositionRepository buffet = new BuffetPositionRepository(connection);
                buffet.addBuffetPosition(dishId,conferenceId,am);
            backToBuffet();
        ...
        ////находим по имени юлда его id

        ///если всё корректно, то делаем запрос на добавление
        ///переходим на главную буфетную
    }

    public void backToBuffet() {
        FXMLLoader fxmlLoader = new FXMLLoader(ConferenceOrganizationCompanyApplication.class.getResource("buffet-menu.fxml"));
        Scene newScene = null;
        try {
            newScene = new Scene(fxmlLoader.load(), 700, 700);
